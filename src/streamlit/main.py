...
st.markdown(
    '<div style="text-align: justify; margin-bottom: 20px;">'
    'A ferramenta escolhida para este projeto foi o Streamlit por diversos motivos. Primeiramente, alguns membros do grupo já possuem experiência com a ferramente, o que facilita a implementação. Além disso, por ser uma ferramenta gratuita, o Streamlit se torna uma opção econômica e inclusiva. Outro fator importante é que Streamlit é baseado em Python, uma linguagem que todos os integrantes dominam, reduzindo significativamente a curva de aprendizagem.'
    '</div>',
    unsafe_allow_html=True
)
...
st.markdown(
    '<div style="text-align: justify; margin-bottom: 20px;">'
    'Os dados utilizados neste projeto foram coletados a partir dos registros de ingressantes da UFPE, abrangendo os anos de 2020 a 2024, porém não foram disponibilizados os dados dos semestres 2021.2 e 2024.2. As informações incluídas nos dados são: origem dos aprovados, campus de destino, cursos escolhidos, e a utilização de cotas, incluindo os diferentes tipos de cotas. A limpeza e organização dos dados foram etapas cruciais para garantir a confiabilidade e a precisão das análises. Utilizamos a biblioteca pandas para manipulação dos dados, removendo linhas com informações em branco e excluindo colunas que não eram relevantes para a análise proposta. A biblioteca openpyxl foi utilizada para trabalhar com arquivos Excel, facilitando a importação e exportação de dados.'
    '</div>',
    unsafe_allow_html=True
)
